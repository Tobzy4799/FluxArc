...
export async function POST(req: Request) {
  ...
  const { name, description, placeholder, price, prompt, developer_wallet, developer_email, tx_hash } = body

  if (!name || !description || !placeholder || !price || !prompt || !developer_wallet || !tx_hash) {
  ...
  if (existing) {
    return NextResponse.json({ success: false, error: 'An agent with this name already exists.' }, { status: 409 })
  }
