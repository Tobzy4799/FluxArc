...
export async function GET() {
  try {
    const { data: jobs, error } = await supabase.from('user_jobs').select('agent_id, rating, review_comment, status, created_at');
    ...
    jobs?.forEach((job) => {
      ...
      if (job.status === 'success' && new Date(job.created_at) > new Date(Date.now() - 86400000)) {
      ...
      if (job.rating) {
        ...
        temp.countRatings += 1;
        stats[job.agent_id].avgRating = Number((temp.sumRatings / temp.countRatings).toFixed(1));
      }
    });
