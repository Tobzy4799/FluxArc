...
const relayerPrivateKey = process.env.BACKEND_RELAYER_PRIVATE_KEY as `0x${string}`;
  ...
  try {
    ...
    if (!system) {
      ...
      if (thirdPartyAgent) {
        ...
        console.log(`[API] Third-party agent detected: ${agentId}, developer wallet: ${thirdPartyDeveloperWallet}`)
      } else {
    ...
    if (!walletSetId) throw new Error("Missing CIRCLE_WALLET_SET_ID configuration.");
    ...
    if (isThirdPartyAgent && thirdPartyDeveloperWallet) {
      ...
      console.log(`[API] Using third-party developer wallet: ${agentWalletAddress}`)
    } else {
      ...
      if (!agentWalletAddress) {
        console.log(`[Circle] Wallet not found. Provisioning clean instance on ARC-TESTNET...`)
    ...
    if (receipt.status === "success") {
      ...
      if (isThirdPartyAgent && thirdPartyDeveloperWallet) {
        ...
        console.log(`[Revenue Split] Agent: ${agentId} | Developer gets: ${developerCut.toFixed(4)} USDC | Platform fee: ${platformCut.toFixed(4)} USDC`)
